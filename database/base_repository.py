from typing import TYPE_CHECKING, TypeVar, Generic, Type, Optional, List, Dict, Any, Tuple
...
class BaseRepository(Generic[T]):
    ...
    def update_field(self, condition: Dict[str, Any], field_name: str, field_value: Any) -> UpdateResult:
        """
        Update a specific field in documents matching the condition.
    ...
    def find_one(self, condition: Dict[str, Any]) -> Optional[T]:
        ...
        """
        doc = self._db._find_one(self._collection_name, condition, projection={"_id": 0})
        if doc:
            try:
