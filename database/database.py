...
class MongoDBInterface:
    ...
    users_groups: UsersGroupRepository

    # This _contest content.
    users_diary: UsersDiaryRepository
    ...
    def __init__(
        ...
        try:
            ...
            self._db.command(command="ping")

            logger.info(
                f"Successfully connected to MongoDB at {host}:{port}, database:"
                " {db_name}"
            )
            ...
            raise ConnectionFailure(
                f"Could not connect to MongoDB at {host}:{port}: {e}"
            )
    ...
    def _init_repositories(self) -> None:
        ...
        self.users_groups = UsersGroupRepository(self)

        self.users_diary = UsersDiaryRepository(self)
...
class _LazyDBInterface:
    ...
    def _ensure_connected(self) -> None:
        ...
        if self._db is None:
            ...
            logger.info(
                f"Initializing MongoDB connection to {host}:{port}, database: "
                "{db_name}.")
