...
@bot.bot.on_callback_query(
    filters.regex(r"^set_timezone_(?P<time_zone>[+-]\d+:\d+)$")
)
async def setting_time_zone_callback(client: Client, call: CallbackQuery):
    ...
    try:
        await call.message.edit(
            text=(
                views.UserSettingsFormatter.to_time_zone_setting(user_settings)
            ),
...
@bot.bot.on_message(
    filters.regex(f"^{keyboards.markup_buttons.SETTING_TIME_ZONE_BUTTON}$")
)
