...
@bot.bot.on_callback_query(
    filters.regex(r"^toggle_setting_(?P<setting_name>.+)$")
)
    ...
    if success:
        ...
        try:
            await call.message.edit(
                text=views.UserSettingsFormatter.to_notify_message(
                    user_settings
                ),
                reply_markup=(
                    keyboards.inline_keyboards.NOTIFICATIONS_SETTITNG_KEYBOARD
                )
            )
        except MessageNotModified:
            pass
    await call.answer(text=constants.messages.SUCCESSFULLY)


@bot.bot.on_message(
    filters.regex(f"^{keyboards.markup_buttons.SETTING_NOTIFICATIONS_BUTTON}$")
)
async def notifications_button(client: Client, message: Message):
    ...
    await message.reply(
        text=views.UserSettingsFormatter.to_notify_message(user_settings),
